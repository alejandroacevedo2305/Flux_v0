...
def objective(trial, el_dia_real, hora_cierre, planificacion_wfm, skills_subsets_x_escr, series, optimizar:str='SLA'):
    ...
    try:
        for j,((k,v),s) in enumerate(zip(planificacion_wfm.items(), skills_subsets_x_escr)):
            ...
            planificacion_optuna[k][0]['inicio']                                = '08:00:00'  
...
for idx, (part, inters) in enumerate(zip(partitions,intervals)):
    ...
    print(f"idx: {idx} study_name: {study_name}")
    IA.optimize(lambda trial: objective(trial, 
                                    el_dia_real = part,
...
planificacion_optima   =  sim.plan_unico([plan for tramo,plan in planificaciones_optimas.items()])
# %%
