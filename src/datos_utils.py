...
class DatasetTTP:
    ...
    def _reshape_df_atenciones(df: pd.DataFrame, resample="60t") -> "pd.DataFrame":
        ...
        medianas["Demanda"] = (
            resampler.count().IdSerie.rename(
                "Demanda"
            )  # Esto es una serie, asi que solo tiene un nombre
    ...
    @staticmethod
    def desde_sql_server(IdOficina: int, engine: "Engine") -> "DatasetTTP":
        # el IdDAtabase es pasado antes, desde otra muestra

        sql_query = f"""-- La verdad esto podria ser un View, pero no sé si eso es mas rapido en MS SQL --
        SELECT
        ...
        WHERE
            ...
            ( IdOficina = {IdOficina} ) AND -- Selecciona solo una oficina, segun arriba
        ...
        df = pd.read_sql_query(
            sql=sql_query, con=engine, parse_dates=[3, 4, 5, 6]
        ).astype(
            {
                "IdOficina": "Int32",
                "IdSerie": "Int8",
    ...
    def un_dia(self, dia: date):
        ...
        return self.atenciones[
            (inicio_dia <= self.atenciones["FH_Emi"])
            & (self.atenciones["FH_Emi"] <= fin_dia)
        ]
